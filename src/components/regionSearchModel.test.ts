...
describe('regionSearchModel', () => {
  it('지역 트리를 평탄화해 시도/시군구/읍면동 미리보기 인덱스를 생성한다', () => {
    const index = buildRegionSearchIndex(regionDataSource)
  ...
  it('부분 일치 검색(`수`/`서`)과 결과 제한 정책을 적용한다', () => {
    const index = buildRegionSearchIndex(regionDataSource)
  ...
  it('검색 결과 레벨별로 SelectedRegionCondition 매핑 규칙을 유지한다', () => {
    const index = buildRegionSearchIndex(regionDataSource)
