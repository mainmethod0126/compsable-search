...
import { KeywordDetailPanel } from '../src/KeywordDetailPanel'
...
describe('KeywordDetailPanel', () => {
  ...
  it('토큰 제거와 전체 삭제 액션을 버튼에 연결한다', async () => {
    const user = userEvent.setup()
    const onRemoveToken = vi.fn()
    const onClearAllTokens = vi.fn()
    ...
    await user.click(screen.getByRole('button', { name: '키워드: react' }))
    await user.click(screen.getByRole('button', { name: '전체 삭제' }))

    expect(onRemoveToken).toHaveBeenCalledWith('keyword:react')
    expect(onClearAllTokens).toHaveBeenCalledTimes(1)
  })
})
