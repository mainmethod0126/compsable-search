...
- 수용 기준:
  - [ ] 조건 변경 시 `options.onChange`가 표준 payload로 호출된다.
  - [ ] 읍/면/동 확정 시 `options.onSelectedEupmyeondong` 호출이 보장된다.
  - [ ] selector 클릭 상호작용에서 `options.onClick`이 누락 없이 실행된다.
  - [ ] 콜백 오류가 발생해도 UI 흐름이 중단되지 않고 오류 처리 정책이 일관되게 적용된다.
