...
- 수용 기준:
  - [ ] `RegionSelectProps`에서 소비자 주입 계약과 내부 주입 계약이 명시적으로 분리된다.
  - [ ] `KeywordSelectProps`를 포함한 selector 타입 경계가 공개 API 기준으로 일관되게 정리된다.
  - [ ] 소비자 샘플 프로젝트에서 타입 체크 실패가 발생하지 않는다.
  - [ ] `0.1.x` 하위 호환 범위와 예외 규칙이 문서에 명시된다.
