...
- 수용 기준:
  - [x] 키워드 토큰이 `selected condition` 모델에 region 조건과 일관된 형태로 저장된다.
  - [x] 키워드 칩 추가/삭제/전체 삭제가 기존 region 칩 정책과 충돌 없이 동작한다.
  - [x] 조건 변경 시 `onChange` payload가 region + keyword 조합 상태를 정확히 반영한다.
  - [x] region/keyword 동시 사용 시 중복, 정렬, 우선순위 정책이 명시된다.
