...
- 수용 기준:
  - [ ] 키워드 관련 공개 타입/props/이벤트 계약이 문서와 코드에 일치한다.
  - [ ] region + keyword 조합 통합 예제와 사용 가이드가 최신 동작 기준으로 제공된다.
  - [ ] trigger-only 모델에서 입력 모델로의 마이그레이션 노트가 제공된다.
  - [ ] 키워드 API 계약 검증 테스트가 CI에 포함된다.
