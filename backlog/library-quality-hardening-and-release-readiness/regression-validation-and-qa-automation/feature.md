...
- 수용 기준:
  - [ ] PRD 핵심 시나리오와 QA-001~QA-006 시나리오가 자동/반자동 테스트로 재현된다.
  - [ ] CI 파이프라인에서 빌드/린트/테스트 결과가 릴리스 게이트로 통합된다.
  - [ ] 실패 시 원인 파악에 필요한 로그/리포트 산출물이 표준화된다.
  - [ ] 릴리스 전 검증 체크리스트가 문서화되고 실행 책임이 정의된다.
