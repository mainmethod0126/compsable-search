...
- 수용 기준:
  - [x] `npm run build`, `npm run lint`가 메인 브랜치 기준으로 100% 성공한다.
  - [x] React `key` 누락 경고를 포함한 고빈도 렌더 경고가 제거된다.
  - [x] 디버그 로그와 불필요 콘솔 출력이 운영 빌드에서 제거된다.
  - [x] 코드 위생 규칙(미사용 import/변수, 타입 경고) 위반이 릴리스 게이트 기준 이하로 유지된다.
