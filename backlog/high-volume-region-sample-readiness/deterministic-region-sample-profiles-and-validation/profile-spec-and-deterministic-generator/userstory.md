...
## 수용 기준
- [ ] 프로파일별 목표 수량과 용도가 문서화되고 미확정 값은 `TBD`로 분리된다.
- [ ] 동일 프로파일 입력에 대해 항상 동일한 데이터셋이 생성된다.
- [ ] 생성 데이터에서 `region.code`가 전역 유일하며 부모-자식 연결 규칙이 유지된다.
- [ ] `DemoService`가 새 생성 규칙을 사용해 프로파일 기반 데이터를 노출한다.
- [ ] 프로파일/생성 규칙 변경 시 검토해야 할 체크리스트가 문서로 제공된다.
