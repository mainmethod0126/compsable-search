...
- 수용 기준:
  - [ ] 프로파일 3종(`small`, `medium`, `large`)이 정의되고 데모에서 앱 재시작 없이 전환된다.
  - [ ] 생성 데이터는 결정적이며 동일 프로파일 호출 시 항상 동일한 결과를 반환한다.
  - [ ] `region.code` 중복이 방지되고, 중복 발생 시 테스트가 실패해 조기 탐지된다.
  - [ ] `large` 프로파일에서 가로 overflow, 클릭 불가, 선택 상태 불일치가 발생하지 않는다.
  - [ ] `onClick`, `onChange`, `onSelectedEupmyeondong` 계약이 대량 샘플에서도 회귀 없이 유지된다.
