...
- 비고/TBD:
  ...
  - FR-023(하위 선택의 상위 지역 색상 표시) 반영: `서울특별시 > 강남구 > 역삼동` 선택 시 시/도 `서울특별시`, 시/군/구 `강남구`에서 하위 선택 존재를 별도 색상으로 표시하고, `current`와 `has-descendant-selected`는 파랑 계열 내에서도 진하기/보더 스타일 차이로 즉시 구분되어야 한다.
