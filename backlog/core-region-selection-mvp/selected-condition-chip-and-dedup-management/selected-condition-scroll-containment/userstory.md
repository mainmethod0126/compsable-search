...
## 수용 기준
- [x] 선택 조건 리스트 콘텐츠 높이가 데스크톱 `144px`, 모바일 `112px` 상한을 초과하면 선택 조건 영역에 내부 세로 스크롤(`overflow-y: auto`)이 활성화된다.
- [x] 스크롤 적용 후에도 개별 삭제/전체 삭제/조건 동기화(`onChange`) 계약이 기존과 동일하게 동작한다.
- [x] 데스크톱(`>=1024px`)과 모바일(`<=768px`)에서 선택 영역이 상위 레이아웃을 과도하게 밀어내지 않는다.
- [x] `ComposableSearch.test.tsx`, `styleIsolation.test.ts` 회귀 검증이 CI 기준으로 통과한다.
- [x] 스크롤 축은 세로 전용으로 유지하고, 헤더 고정(sticky)은 미적용으로 관리하며 재검토 오너를 `FE Lead/PO`로 추적한다.
