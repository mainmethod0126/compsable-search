...
## 수용 기준
- [ ] `서울특별시 > 강남구 > 역삼동` 선택 시 시/도 컬럼 `서울특별시`, 시/군/구 컬럼 `강남구`는 하위 선택 존재를 나타내는 별도 색상 상태(`has-descendant-selected`)로 표시된다.
- [ ] `서울특별시 > 강남구 전체`처럼 하위 `전체` 항목 선택 시에도 시/도 컬럼 `서울특별시`, 시/군/구 컬럼 `강남구`가 `has-descendant-selected`로 표시된다.
- [ ] `has-descendant-selected` 색상은 기본/`current`/직접 `selected` 상태와 시각적으로 구분된다. 특히 `current`와 `has-descendant-selected`가 모두 파랑 계열일 때도 진하기/채도 또는 보더 스타일 차이로 즉시 구분된다.
- [ ] 하위 선택(`역삼동`)이 해제되면 상위(`서울특별시`, `강남구`)의 `has-descendant-selected` 상태는 즉시 제거된다.
- [ ] `has-descendant-selected` 상태 추가가 FR-016(시각 상태 표기), FR-022(타이포그래피 일관성), FR-019/FR-021(상호 배타/자동 선택 금지) 동작을 깨지 않음을 검증한다.
