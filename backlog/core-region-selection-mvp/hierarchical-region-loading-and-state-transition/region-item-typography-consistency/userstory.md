...
## 수용 기준
- [x] 시/군/구 컬럼의 `시/도 전체` 체크박스 라벨 텍스트와 일반 시/군/구 item 텍스트는 동일한 `font-family`, `font-size`, `font-weight`, `line-height`를 사용한다.
- [x] 읍/면/동 체크박스 라벨 텍스트와 시/도/시/군/구 일반 item 텍스트는 동일한 타이포그래피 기준을 따른다.
- [x] `selected/current/hover` 상태에서도 타이포그래피 값은 유지되고, 강조는 색상/배경 등 비타이포 속성으로만 구분된다.
- [x] 타이포그래피 정합성 변경이 FR-016(시각 상태 표기)과 기존 선택 토글 동작(FR-007~FR-021)을 깨지 않음을 검증한다.
