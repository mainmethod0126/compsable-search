...
- 수용 기준:
  - [ ] `region`, `keyword`, `sort` selector가 정의된 순서대로 항상 렌더링된다.
  - [ ] region 트리거 클릭 시 상세 패널 열기/닫기가 토글되고, 재오픈 시 직전 선택 컨텍스트를 유지한다.
  - [ ] selector 콘텐츠 주입 지점이 문서화된 계약대로 렌더링되어 확장 selector 추가 시 기존 동작이 깨지지 않는다.
  - [ ] 상세 콘텐츠가 없는 상태에서 빈 영역 처리 정책(placeholder 또는 미노출)이 일관되게 동작한다.
