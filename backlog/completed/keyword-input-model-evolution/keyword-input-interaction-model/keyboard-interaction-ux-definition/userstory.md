...
## 수용 기준
- [x] `Enter` 입력 시 현재 텍스트가 토큰으로 확정되며, 빈 입력에 대한 처리 정책이 정의된다.
- [x] 입력창이 비어 있을 때 `Backspace` 동작(마지막 토큰 선택/삭제)의 정책이 일관되게 동작한다.
- [x] `Tab`/`Shift+Tab` 기반 포커스 이동 순서와 시각적 포커스 표시 기준이 정의된다.
- [x] 접근성 기본 항목(`label`, 안내 텍스트, 키보드 사용 힌트)이 UX 명세와 구현에 반영된다.
