...
- 수용 기준:
  - [x] 입력, 엔터 확정, 백스페이스 삭제를 포함한 핵심 입력 상호작용이 일관되게 동작한다.
  - [x] 토큰 정규화(공백 정리, 중복 기준, 길이 제한)가 명시된 규칙대로 적용된다.
  - [x] 최대 토큰 개수/입력 제약 정책이 구성 가능하며 기본값이 문서화된다.
  - [x] 접근성 기본 항목(`label`, 포커스 흐름, 안내 텍스트)이 정의된다.
