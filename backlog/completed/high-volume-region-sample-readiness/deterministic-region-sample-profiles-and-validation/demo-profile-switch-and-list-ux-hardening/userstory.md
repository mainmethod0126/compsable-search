...
## 수용 기준
- [x] 데모 UI에서 `small`, `medium`, `large` 전환이 앱 재실행 없이 동작한다.
- [x] `large` 프로파일에서 가로 overflow, 클릭 불가, 선택 표시 불일치가 발생하지 않는다.
- [x] 모바일(`<=768px`)과 데스크톱(`>=1024px`) 모두에서 열기/선택/해제/전체 선택 조작이 가능하다.
- [x] 프로파일 전환 후에도 `onClick`, `onChange`, `onSelectedEupmyeondong` 콜백 계약이 유지된다.
- [x] 프로파일 전환 시 초기화/유지 정책(선택 상태, 현재 컬럼 포커스)이 정의된 규칙대로 동작한다.
