...
## 수용 기준
- [x] 검색 입력이 `cs-selector-area` 하위 좌상단에 표시되고 `cs-detailed-area` 바깥 배치 계약을 만족하며 좌측 SVG 아이콘 슬롯이 동작한다.
- [x] 입력값과 부분 일치하는 미리보기 목록이 즉시 갱신된다.
- [x] 미리보기 항목은 경로 문자열 형식으로 표시된다.
- [x] 미리보기 클릭 시 선택 조건 칩이 정확히 1회 추가되고 기존 중복/상호배타 규칙이 유지된다.
- [x] `onChange`, `onSelectedEupmyeondong` 콜백 계약이 기존 동작과 호환된다.
