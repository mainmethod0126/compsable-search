...
- 수용 기준:
  - [x] 지역 검색 컴포넌트가 `cs-selector-area` 하위 좌상단에 렌더링되고 `cs-detailed-area` 바깥 배치 계약을 만족한다.
  - [x] 시/도/시군구/읍면동 전체를 포함한 검색 인덱스가 생성되고 부분 일치 검색이 동작한다.
  - [x] 미리보기 항목이 경로 문자열(`시도 > 시군구 > 읍면동`) 규칙으로 표시된다.
  - [x] 미리보기 클릭 시 선택 조건 칩 반영, 중복/상호배타 정책, 콜백 계약이 유지된다.
  - [x] unit/integration/regression 테스트와 운영 관측/롤백 준비가 완료된다.
