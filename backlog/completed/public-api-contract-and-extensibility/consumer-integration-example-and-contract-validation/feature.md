...
- 수용 기준:
  - [x] region/keyword 조합 시나리오를 포함한 소비자 통합 예제가 최신 공개 API 기준으로 동작한다.
  - [x] API 문서 코드 예제가 CI에서 타입 체크 또는 실행 검증된다.
  - [x] 계약 기반 테스트(필수/선택 props, 콜백 payload)가 추가되어 회귀를 탐지한다.
  - [x] API 문서-구현 불일치 이슈가 `0건`으로 유지된다.
