...
## 수용 기준
- [ ] 시/도/시군구/읍면동 레벨을 모두 포함하는 검색 인덱스 구조가 정의된다.
- [ ] 한 글자 입력(`수`, `서`) 부분 일치 케이스가 테스트로 보장된다.
- [ ] 상위 레벨 선택 항목 매핑(시도/시군구 클릭 시 조건 구성) 규칙이 문서화되고 테스트된다.
- [ ] 검색 모델 단위 테스트가 Red -> Green -> Refactor 흐름으로 통과한다.
