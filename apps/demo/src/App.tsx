...
interface DemoSelectionItem {
  id: string
}

interface DemoValueChangeMeta {
  reason?: string
  source?: string
  selectorType?: string
  selectorId?: string
...
function formatOnValueChangeMessage(
  selectedItems: DemoSelectionItem[],
  meta: DemoValueChangeMeta,
): string {
...
function formatOnSelectedMessage(selected: DemoRegion): string {
...
function App() {
  ...
  const handleValueChange = useCallback(
    (nextValue: DemoSelectionItem[], meta: DemoValueChangeMeta) => {
  ...
  const selectors = useMemo(
    () => [
      createRegionSelector('demo-region-selector', {
        ...activeDataSource,
        options: {
          placeholder: '지역 선택',
    ...
    <main className="demo-root demo-shell">
      <header>
        ...
        <p className="demo-description">
          workspace package 기준 상호작용 회귀 검증용 독립 데모 앱
        </p>
      </header>
      <section className="demo-profile-panel">
        ...
        <p className="demo-profile-summary">
          규모: 시/도 {activeProfileSpec.sidoCount}개, 시/군/구{' '}
          {activeProfileSpec.sigunguPerSido}개/시도, 읍/면/동{' '}
      ...
      <section className="demo-metrics-panel" data-testid="demo-performance-metrics">
        ...
        <p className="demo-metrics-item">
          region.onSelectedEupmyeondong 호출: {activeProfileMetrics.regionOnSelectedCount}
          회
        </p>
      </section>
