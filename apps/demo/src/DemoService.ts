...
export interface DemoRegionDataSource {
  findAllSidos: (context?: SelectorLoadContext) => MaybePromise<DemoRegion[]>
  findAllSigungus: (
    sidoCode: string,
    context?: SelectorLoadContext,
  ) => MaybePromise<DemoRegion[]>
...
interface DemoRegionDataset {
  sidos: DemoRegion[]
  sigungusBySidoCode: Record<string, DemoRegion[]>
  eupmyeondongsBySigunguCode: Record<string, DemoRegion[]>
}
...
function createRegion(displayName: string, code: string): DemoRegion {
  return {
    displayName,
    name: displayName,
    code,
...
function cloneRegions(regions: DemoRegion[]): DemoRegion[] {
...
function createDemoRegionDataset(spec: DemoRegionProfileSpec): DemoRegionDataset {
  const sidos: DemoRegion[] = []
  const sigungusBySidoCode: Record<string, DemoRegion[]> = {}
  const eupmyeondongsBySigunguCode: Record<string, DemoRegion[]> = {}
  ...
  for (let sidoIndex = 0; sidoIndex < spec.sidoCount; sidoIndex += 1) {
    ...
    const sido = createRegion(createRegionDisplayName('sido', { sidoOrdinal }), sidoCode)

    sidos.push(sido)
    sigungusBySidoCode[sidoCode] = []

    for (let sigunguIndex = 0; sigunguIndex < spec.sigunguPerSido; sigunguIndex += 1) {
...
function createDataSourceFromDataset(dataset: DemoRegionDataset): DemoRegionDataSource {
  return createDataSourceFromDatasetByMode(dataset, 'sync')
}

function createDataSourceFromDatasetByMode(
  dataset: DemoRegionDataset,
  mode: DemoRegionDataSourceMode,
): DemoRegionDataSource {
  ...
  return {
    findAllSidos: (context?: SelectorLoadContext) => resolveRegions(dataset.sidos, context),
    findAllSigungus: (sidoCode: string, context?: SelectorLoadContext) =>
      resolveRegions(dataset.sigungusBySidoCode[sidoCode] ?? [], context),
    findAllEupmyeondongs: (sigunguCode: string, context?: SelectorLoadContext) =>
