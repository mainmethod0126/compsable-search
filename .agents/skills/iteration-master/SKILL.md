...
description: PRD와 백로그(Epic/Feature/UserStory/Task)를 분석해 이터레이션(스프린트) 계획과 실행 체크리스트를 생성하고 갱신한다. `PRD.md`, `PRD.en.md`, `backlog/**/epic.md|feature.md|userstory.md|task.md`를 근거로 범위, 우선순위, 의존성, 완료 조건을 정리해야 할 때 사용한다. 작업 완료 시 체크리스트를 `[ ]`에서 `[x]`로 갱신하고 완료 근거를 기록해야 할 때 사용한다.
---
...
- PRD와 백로그를 근거로 실행 가능한 이터레이션 계획을 수립한다.
- 작업자 실행용 체크리스트를 만들고 완료 상태와 근거를 갱신한다.
...
- PRD 목표/수용 기준과 백로그의 `P0/P1`, 의존성, 리스크를 기준으로 후보를 선정한다.
- 계층 추적성을 유지한다.
- `Epic -> Feature -> UserStory -> Task` 연결이 끊기지 않게 한다.
- 이터레이션에 포함한 각 항목의 원본 경로를 기록한다.
