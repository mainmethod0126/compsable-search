...
- Feature: Epic을 구현 가능한 기능 단위로 쪼갠다. Task 이슈를 하위로 연결한다.
- Task: 단일 담당자가 완료 가능한 실행 단위. 선행/후행 Task 링크를 포함한다.
...
- 최소 라벨: `type:*` + `priority:*` + `area:*` 1개 이상
...
3. Task 이슈를 생성하며 상위 Feature 링크와 선행 Task 링크를 본문에 기록한다.
4. 생성된 URL/번호를 `task-breakdown.md` 표에 즉시 반영한다.
5. Epic/Feature 본문에 하위 이슈 체크리스트를 추가해 탐색 비용을 줄인다.
...
- 실패한 항목은 `task-breakdown.md`의 GitHub 매핑 섹션에 `TBD(실패 원인)`으로 남긴다.
- 재시도 전 권한/레이트리밋/필수 필드 누락 여부를 먼저 점검한다.

## 8. MCP 미사용 시 대체

- GitHub MCP가 없으면 문서 산출(`task-breakdown.md`)까지만 완료한다.
- 후속 업로드를 위해 Epic/Feature/Task별 제목, 본문, 라벨 후보를 문서에 포함한다.
