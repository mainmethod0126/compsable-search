...
- [ ] 커밋 범위의 Task(T-169~T-172)가 체크리스트에서 근거와 함께 완료 처리된다.
- [ ] 기능/스타일 회귀 테스트 결과가 통과한다.
- [ ] 운영 준비(관측/롤백/수동 검증)가 문서로 준비된다.
- [ ] 미해결 이슈가 릴리스 허용 범위 내로 정리된다.

## 7. 오픈 이슈
- 스크롤 임계치(예: 20/50/200) 최종 기준 확정 필요 (`TBD`)
- 높이 정책(px 고정 vs viewport 비율) 확정 필요 (`TBD`)
- 헤더 sticky 적용 여부 확정 필요 (`TBD`)
