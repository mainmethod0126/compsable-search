...
- backlog/core-region-selection-mvp/hierarchical-region-loading-and-state-transition/parent-region-descendant-selection-indicator/userstory.md
...
- 기술 목표:
  ...
  - FR-023 확장 요구(하위 `전체` 선택 시 상위 item 파란 계열 하이라이팅)를 US-039/T-153~T-156으로 반영해 하위 선택 가시성을 고정한다.
  - E-03/F-08을 완료해 PRD 핵심 시나리오와 QA 시나리오의 회귀 검증 표준 경로를 확보한다.
- 이번 이터레이션 성공 기준:
  - 포함 범위 Task(T-013~T-036, T-085~T-096, T-153~T-156)가 체크리스트에서 근거와 함께 완료 처리된다.
  - AC-020(시/군/구 `서울특별시 전체` 직접 체크 + 읍/면/동 중복 미노출)이 검증 시나리오에 반영된다.
  - AC-023(`서울특별시 > 강남구 전체`를 포함한 하위 `전체` 선택의 상위 하이라이팅)가 검증 시나리오에 반영된다.
...
| P1 | [E-01] | [F-02] | [US-039] | [T-153] 정의한다: 하위 선택 상위 색상 인디케이터 범위와 상태 우선순위 | backlog/core-region-selection-mvp/hierarchical-region-loading-and-state-transition/parent-region-descendant-selection-indicator/define-scope-and-acceptance/task.md |
| P1 | [E-01] | [F-02] | [US-039] | [T-154] 구현한다: 하위 선택 상위 색상 인디케이터 핵심 시나리오 | backlog/core-region-selection-mvp/hierarchical-region-loading-and-state-transition/parent-region-descendant-selection-indicator/implement-main-scenario/task.md |
| P1 | [E-01] | [F-02] | [US-039] | [T-155] 검증한다: 하위 선택 상위 색상 인디케이터 회귀와 실패 시나리오 | backlog/core-region-selection-mvp/hierarchical-region-loading-and-state-transition/parent-region-descendant-selection-indicator/validate-regression-and-failure-flow/task.md |
| P1 | [E-01] | [F-02] | [US-039] | [T-156] 준비한다: 하위 선택 상위 색상 인디케이터 관측과 릴리스 운영 | backlog/core-region-selection-mvp/hierarchical-region-loading-and-state-transition/parent-region-descendant-selection-indicator/prepare-observability-and-rollout/task.md |
...
| 2026-03-04 (수) | 프론트엔드 테크리드 | T-153 | FR-023 하위 `전체` 하이라이팅 범위/상태 우선순위 확정 |
| 2026-03-05 (목) | 프론트엔드 엔지니어 | T-154 | T-153 완료 후 착수 |
...
| 2026-03-07 (토) | QA 엔지니어 | T-155 | T-154 완료본 기준 FR-023 확장 케이스 검증 착수 |
...
| Track-E | E-01/F-02 FR-023 확장 (T-153~T-156) | FE Tech Lead + FE 엔지니어 + QA + 릴리스 | 2026-03-10 |
...
| 리스크 | 하위 `전체` 선택(`서울특별시 전체`, `강남구 전체`)에서 상위 item 하이라이팅 누락 재발 가능성 | Medium | T-155/QA-011에 `하위 전체` 케이스를 고정하고 T-156 스모크 체크리스트에 포함 | PM/FE/QA |
| 리스크 | 테스트 자동화 파이프라인 환경 편차(CI/로컬) | Medium | 실패 로그 포맷 표준화와 재현 스크립트 동시 관리 | DevOps |
| 리스크 | 40개 Task 범위로 인한 검증 병목 | Medium | QA 착수 시점을 3회 분할(3/5, 3/6, 3/7)하고 재검증 버퍼 확보 | QA Lead |
